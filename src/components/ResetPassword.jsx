...
const ResetPassword = () => {
  ...
  async function SubmitHandler(data) {
    ...
    try {
      ...
      // Example API call
      // const res = await apiConnecter("post", "Auth/ResetPassword", { OTP: data.otp, NewPassword: data.newPassword });
      // console.log(res);
      if (ref.current) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#1a202c]">
      ...
      <div className="p-8 rounded-lg shadow-lg bg-[#1a202c] ring-2 ring-[#1a202c] w-80 transform transition-transform duration-500 hover:scale-105">
        ...
        <form onSubmit={handleSubmit(SubmitHandler)}>
          <div className="mb-4">
            <label htmlFor="otp" className="block text-left text-white font-semibold mb-2">OTP</label>
            <input
              type="text"
              id="otp"
              required
          ...
          <div className="mb-4">
            <label htmlFor="newPassword" className="block text-left text-white font-semibold mb-2">New Password</label>
            <input
              type="password"
              id="newPassword"
              required
